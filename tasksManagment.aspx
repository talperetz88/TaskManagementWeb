<%@ Page Language="C#" AutoEventWireup="true" CodeFile="tasksManagment.aspx.cs" Inherits="tasks" %>
    ...
    <form id="form1" runat="server">
       <div class="panel">
        ...
        <h4>רשימת עובדים</h4>
            ...
            <asp:Button ID="Button2" runat="server" Text="הוספה" Width="168px" OnClick="Button2_Click" />
        ...
        <br />
            <asp:GridView ID="GridView4" CssClass="table table-hover table-striped" Width="450px" HorizontalAlign="Center" runat="server" AllowPaging="True" AllowSorting="True" AutoGenerateColumns="False" CellPadding="4" DataSourceID="SqlDataSource3" ForeColor="#333333" GridLines="None">
                <AlternatingRowStyle BackColor="White" />
                <Columns>
                    <asp:BoundField DataField="WorkerName" HeaderText="שם העובד" SortExpression="WorkerName" />
                    ...
                    <asp:CommandField ShowDeleteButton="True" ShowEditButton="True" ButtonType="Button" CancelText="ביטול" DeleteText="מחיקה" EditText="עריכה" UpdateText="עדכון" />
                </Columns>
                <EditRowStyle BackColor="#2461BF" />
                <FooterStyle BackColor="#507CD1" Font-Bold="True" ForeColor="White" />
                <HeaderStyle BackColor="#507CD1" Font-Bold="True" ForeColor="White" />
            ...
            <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConflictDetection="CompareAllValues" ConnectionString="<%$ ConnectionStrings:loginConnectionString1 %>"
                 DeleteCommand="DELETE FROM taskForWorker FROM Table1 INNER JOIN taskForWorker ON Table1.Name = taskForWorker.WorkerName INNER JOIN Tasks ON Tasks.Task = taskForWorker.Task WHERE (taskForWorker.[Task] = @original_Task)" 
                 OldValuesParameterFormatString="original_{0}"
                 SelectCommand="SELECT taskForWorker.WorkerName, taskForWorker.Task, taskForWorker.Description, taskForWorker.status FROM Table1 INNER JOIN taskForWorker ON Table1.Name = taskForWorker.WorkerName INNER JOIN Tasks ON Tasks.Task = taskForWorker.Task" 
                UpdateCommand="UPDATE taskForWorker SET status = @status FROM Table1 INNER JOIN taskForWorker ON Table1.Name = taskForWorker.WorkerName INNER JOIN Tasks ON Tasks.Task = taskForWorker.Task WHERE (taskForWorker.Task = @Task)">
                <DeleteParameters>
                    <asp:Parameter Name="original_Task" Type="String" />
                </DeleteParameters>
                <UpdateParameters>
            ...
            <div style="width: 90%; margin-right: 5%; margin-left: 5%; text-align: center">
               <h4>רשימת מטלות לעובדים</h4>
                 <asp:GridView ID="GridView5" runat="server" CssClass="table table-hover table-striped" Width="450px" HorizontalAlign="Center" AllowPaging="True" AllowSorting="True" AutoGenerateColumns="False" CellPadding="4" DataSourceID="SqlDataSource1" ForeColor="#333333" GridLines="None">
                    <AlternatingRowStyle BackColor="White" />
                    <Columns>
                        <asp:BoundField DataField="WorkerName" HeaderText="שם העובד" ReadOnly="True" SortExpression="WorkerName" />
                        <asp:BoundField DataField="Task" HeaderText="שם המטלה" SortExpression="Task" />
                        <asp:BoundField DataField="Description" HeaderText="תיאור המטלה" ReadOnly="True" SortExpression="Description" />
                        <asp:BoundField DataField="status" HeaderText="סטטוס" SortExpression="status" />
                        <asp:CommandField ButtonType="Button" CancelText="ביטול" DeleteText="מחיקה" EditText="עריכה" ShowDeleteButton="True" ShowEditButton="True" UpdateText="עדכון" />
                    </Columns>
                    <EditRowStyle BackColor="#2461BF" />
                    <FooterStyle BackColor="#507CD1" Font-Bold="True" ForeColor="White" />
                    <HeaderStyle BackColor="#507CD1" Font-Bold="True" ForeColor="White" />
            ...
            <asp:Button ID="addTaskBtn" runat="server" OnClick="addTaskBtn_click" Text="הוספה" Width="168px" />
